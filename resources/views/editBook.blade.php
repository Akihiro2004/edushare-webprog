...
<div class="bg-gradient-to-br from-blue-950 to-slate-900 min-h-screen">
    <div class="max-w-6xl mx-auto px-6 py-20 relative">
        ...
        <div class="relative text-center mb-12">
            ...
            <h1 class="text-4xl font-bold text-white mb-4">Edit Book</h1>
            <p class="text-gray-300 max-w-2xl mx-auto">
                Update the book information in our library collection. Modify the details below to update the catalog.
            </p>
        </div>

        <form action="{{ route('updateBook', ['book' => $book->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="relative backdrop-blur-xl bg-white/5 rounded-3xl border border-white/10 overflow-hidden">
                        ...
                        <div class="col-span-12 lg:col-span-4">
                            <div class="space-y-3">
                                ...
                                <div class="relative group aspect-[3/4] border-2 border-dashed border-white/20 rounded-2xl
                                    ...
                                    <div id="defaultUploadContent" class="absolute inset-0 flex flex-col items-center justify-center p-6 {{ $book->thumbnail ? 'hidden' : '' }}">
                                    ...
                                    <div id="previewContainer" class="{{ $book->thumbnail ? '' : 'hidden' }} absolute inset-0">
                                        <img id="imagePreview"
                                        ...
                                        <div class="absolute bottom-0 inset-x-0 bg-black/50 p-3 text-center">
                                            <p id="fileName" class="text-sm text-white truncate">{{ $book->thumbnail ? basename($book->thumbnail) : '' }}</p>
                                        </div>
                                    </div>
                            ...
                            @error('thumbnail')
                                <div class="invalid-feedback text-white/50">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-12 lg:col-span-8 space-y-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <textarea name="description" required rows="4"
                                          class="w-full px-4 py-3.5 rounded-xl bg-white/5 border border-white/10
                                                 ...
                                                 focus:outline-none focus:border-white/30 focus:ring-4 focus:ring-white/10
                                                 hover:border-white/20 hover:bg-white/[0.075]
                                                 transition-all duration-300">{{ old('description', $book->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback text-white/50">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                ...
                                <div class="col-span-2 md:col-span-1 space-y-3">
                                    ...
                                    <input type="file" name="book_path" accept=".pdf,.epub,.mobi"
                                           class="block w-full text-sm text-white/60
                                                  ...
                                                  transition-all duration-300">
                                    @if($book->book_path)
                                        <p class="text-xs text-white/50 mt-2">Current file: {{ basename($book->book_path) }}</p>
                                    @endif
                ...
                <div class="relative mt-4">
                    ...
                    <div class="px-10 py-6 relative flex items-center justify-between">
                        <p class="text-sm text-white/50">Leave file inputs empty to keep current files</p>
                        <button type="submit"
                                class="group inline-flex items-center gap-3 px-8 py-3.5
                                       ...
                                       backdrop-blur-sm
                                       transition-all duration-300">
                            <span class="text-sm font-medium text-white">Update Book</span>
